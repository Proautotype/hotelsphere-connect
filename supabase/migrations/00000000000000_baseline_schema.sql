...
CREATE FUNCTION public.enforce_hotel_not_platform_team() RETURNS trigger
...
CREATE FUNCTION public.enforce_member_not_platform_team() RETURNS trigger
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
BEGIN
...
CREATE FUNCTION public.enforce_role_separation() RETURNS trigger
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
BEGIN
...
CREATE FUNCTION public.has_hotel_access(_hotel_id uuid) RETURNS boolean
...
CREATE FUNCTION public.hotel_commission_percent(_hotel_id uuid) RETURNS numeric
    LANGUAGE sql STABLE SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
  SELECT COALESCE(
...
CREATE FUNCTION public.hotel_is_public(_hotel_id uuid) RETURNS boolean
    LANGUAGE sql STABLE SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
...
CREATE FUNCTION public.is_demo_hotel(_hotel_id uuid) RETURNS boolean
    LANGUAGE sql STABLE SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
...
CREATE FUNCTION public.is_platform_team() RETURNS boolean
    LANGUAGE sql STABLE SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
  SELECT EXISTS (
...
CREATE FUNCTION public.owns_hotel(_hotel_id uuid) RETURNS boolean
    LANGUAGE sql STABLE SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
...
CREATE FUNCTION public.seed_platform_admin(target_email text) RETURNS void
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
DECLARE
...
CREATE FUNCTION public.user_is_hotel_person(_user_id uuid) RETURNS boolean
    LANGUAGE sql STABLE SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
...
CREATE FUNCTION public.user_is_platform_team(_user_id uuid) RETURNS boolean
    LANGUAGE sql STABLE SECURITY DEFINER
    SET search_path TO 'public'
    AS $$
  SELECT EXISTS (
...
GRANT SELECT,INSERT ON TABLE public.ad_requests TO sandbox_exec;
...
GRANT SELECT,USAGE ON SEQUENCE public.booking_reference_seq TO sandbox_exec;
...
GRANT SELECT,INSERT ON TABLE public.cash_sessions TO sandbox_exec;
...
GRANT SELECT,INSERT ON TABLE public.data_requests TO sandbox_exec;
...
GRANT SELECT,INSERT ON TABLE public.expenses TO sandbox_exec;
...
GRANT SELECT,INSERT ON TABLE public.folio_items TO sandbox_exec;
...
GRANT SELECT,INSERT ON TABLE public.guests TO sandbox_exec;
...
GRANT SELECT,INSERT ON TABLE public.hotel_members TO sandbox_exec;
...
GRANT SELECT,INSERT ON TABLE public.hotel_payment_methods TO sandbox_exec;
...
GRANT SELECT,INSERT ON TABLE public.hotel_subscriptions TO sandbox_exec;
...
GRANT SELECT,INSERT ON TABLE public.hotels TO sandbox_exec;
...
GRANT SELECT,USAGE ON SEQUENCE public.payment_reference_seq TO sandbox_exec;
...
GRANT SELECT,USAGE ON SEQUENCE public.receipt_number_seq TO sandbox_exec;
...
GRANT SELECT,INSERT ON TABLE public.room_types TO sandbox_exec;
...
GRANT SELECT,INSERT ON TABLE public.rooms TO sandbox_exec;
