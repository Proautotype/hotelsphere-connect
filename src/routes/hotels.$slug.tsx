...
export const Route = createFileRoute("/hotels/$slug")({
  loader: async ({ params }) => {
    const result = await getPublicHotel({ data: { slug: params.slug } });
    if (!result) throw notFound();
    return result;
  },
  head: ({ loaderData }) => {
    const name = loaderData?.hotel?.name ?? "Hotel";
    const city = loaderData?.hotel?.city ?? "Ghana";
    const title = `${name}, ${city} — Book on Custard Hotels`;
  ...
  notFoundComponent: () => (
    <DiscoveryLayout>
      <p className="ink bg-card p-6 font-medium">That hotel isn't listed publicly.</p>
    </DiscoveryLayout>
  ),
  errorComponent: () => (
    <DiscoveryLayout>
      ...
      <div className="ink shadow-hard overflow-hidden bg-card">
        ...
        <div className="p-5 sm:p-8">
          ...
          <div className="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-sm text-muted-foreground">
            ...
            <span className="flex items-center gap-1">
              <Clock className="size-4" aria-hidden /> Check-in {String(hotel.check_in_time).slice(0, 5)} · out {String(hotel.check_out_time).slice(0, 5)}
            </span>
          </div>
                      ...
                      {hotel.show_prices && (
                        <>
                          ...
                          <p className="mt-2 text-sm font-medium">
                            {money(room.quote.total, hotel.currency)} total · {room.quote.nights} night{room.quote.nights === 1 ? "" : "s"}
                          </p>
                        </>
                      )}
                      <Button
            ...
            {!hotel.accept_online_bookings ? (
              ...
              <div className="mt-4 space-y-3">
                ...
                <p className="text-xs text-muted-foreground">
                  You'll be taken to Mobile Money / card checkout. If online payment isn't available you can settle at the front desk.
                </p>
              </div>
            )}
            {hotel.cancellation_policy && <p className="mt-4 border-t-[2px] border-ink pt-3 text-xs text-muted-foreground">{hotel.cancellation_policy}</p>}
          </div>
        </aside>
      </div>
    </DiscoveryLayout>
